...
How large can a PDF be before a government, university or job portal rejects it? This dataset records the published file size limit of 180 portals in France, the United States, the United Kingdom, Germany, Spain and India, each with the exact wording of the limit and a link to the official page where it was found.

It was first collected in September 2026 for the EasyPDF study [The 1 MB Problem: PDF File Size Statistics for 2026](https://www.easypdf.fr/blog/pdf-file-size-statistics-2026) (French version: [Le problème du 1 Mo](https://www.easypdf.fr/fr/blog/statistiques-taille-fichier-pdf-2026)). Version 1.1.0 (September 25, 2026) extends the United States to 36 portals, all rechecked, with a page per portal at [easypdf.fr/pdf-size-limits](https://www.easypdf.fr/pdf-size-limits).
...
| United States | 36 | 27 | 7 | 1 MB | 15 MB | 100 MB |
...
- In version 1.0 (18 U.S. entries), the United States and the United Kingdom taken together (18 published caps) had a median of 10 MB, the figure used in the study.
- 62 of the 181 entries publish no limit at all: the user finds out when the upload fails.
...
- LinkedIn publishes a recommended size (2 MB), not a limit, and Grants.gov a suggested maximum for a whole application package (200 MB). Their entries are kept in the data with `cap_type = recommendation` and are left out of the medians.

The dataset has 181 entries for 180 portals because the French ANTS agency publishes two different caps for two services.

## Files

| File | Content |
|---|---|
| `data/all-portals.csv` | All 181 entries, one row per portal and usage |
